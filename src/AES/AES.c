...
void ShiftRows(int data[])
{
  int i,j,i4;
  unsigned char *cb=(unsigned char*)data;
  unsigned char cw[NBb];// Nat8で16byteとる
  memcpy(cw,cb,sizeof(cw));//dataの先頭16byteをコピーする,　４ワード、配列の終端子の分もコピーしている？ 
  /*
  int a[10];
...
int dataget(void* data,int n)
{
  ...
  /*
  dataはint型の配列
  それをvoid型のポインタで受け取って，unsinged charのポインタ(1 byteごと)で参照して，
  int型へキャストしていないので，int型として解釈するとそこから4byte参照れるはず．．
  */
}
